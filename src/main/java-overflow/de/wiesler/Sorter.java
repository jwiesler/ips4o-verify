...
public final class Sorter {
    /*@ public normal_behaviour
      ...
      @ requires_free Constants.ACTUAL_BASE_CASE_SIZE < end - begin <= Buffers.MAX_LEN;
      @ requires_free \invariant_free_for(storage);
      @ requires_free \disjoint(storage.allArrays, values[*]);
      ...
      @ ensures_free \result.isValidForLen(end - begin);
      @ ensures_free Functions.isSortedSlice(values, begin, begin + \result.num_samples);
    ...
    private static SampleParameters sample(int[] values, int begin, int end, Storage storage) {
        ...
        /*@ normal_behaviour
          @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), before_sort);
          @ measured_by end - begin, 0;
          @ assignable_free \strictly_nothing;
          @*/
        {;;}
    ...
    /*@ public normal_behaviour
      ...
      @ requires \invariant_for(storage);
      @
      @ requires_free \disjoint(values[*], bucket_starts[*], storage.allArrays);
      @
      @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), \old(\dl_seq_def_workaround(begin, end, values)));
      @ ensures_free \result != null ==>
    ...
    private static /*@ nullable */ PartitionResult partition(
        ...
        /*@ normal_behaviour
          @ ensures_free before_from_sorted_samples == before_copy_unique;
          @ assignable_free \strictly_nothing;
          @ measured_by end - begin, 1;
          @*/
        {;;}
        /*@ normal_behaviour
          @ ensures_free \dl_seqPerm(before_from_sorted_samples, oldValues);
          @ assignable_free \strictly_nothing;
          @ measured_by end - begin, 1;
          @*/
        {;;}
        ...
        /*@ normal_behaviour
          @ ensures_free valuesBeforePartition == before_from_sorted_samples;
          @ ensures_free \invariant_free_for(classifier);
        ...
        {;;}
        /*@ normal_behaviour
          @ ensures_free \dl_seqPerm(valuesBeforePartition, oldValues);
          @ ensures_free (\exists int i; begin <= i < end; (\exists int j; begin <= j < end; classifier.classOf(values[i]) != classifier.classOf(values[j])));
          @ assignable_free \strictly_nothing;
          @ measured_by end - begin, 1;
          @*/
        {;;}
        ...
        /*@ normal_behaviour
          @ ensures_free bucketIndexFromOffset(bucket_starts, classifier.num_buckets, end - begin);
          @ ensures_free (\exists int i; 0 <= i < valuesAfterPartition.length;
        ...
        /*@ normal_behaviour
          @ ensures_free (\exists int i; begin <= i < end; (\exists int j; begin <= j < end; classifier.classOf(values[i]) != classifier.classOf(values[j])));
          @ ensures_free nonEmptyBucketsLemma(classifier, values, begin, end, bucket_starts);
          @ ensures_free equalityBucketsLemma(classifier, values, begin, end, bucket_starts);
          @ assignable_free \strictly_nothing;
          @ measured_by end - begin, 1;
          @*/
        {;;}
        ...
        /*@ normal_behaviour
          @ ensures_free notAllValuesInOneBucketLemma(bucket_starts, classifier.num_buckets, end - begin);
          @ ensures_free allBucketsPartitionedLemma(classifier, values, begin, end, bucket_starts);
          @ assignable_free \strictly_nothing;
          @ measured_by end - begin, 1;
          @*/
        {;;}
    ...
    /*@ public normal_behaviour
      @ requires_free 0 <= begin <= end <= values.length;
      @ requires_free end - begin <= Buffers.MAX_LEN;
      @ requires_free 0 <= bucket && bucket < num_buckets;
      @ requires_free Functions.isValidBucketStarts(bucket_starts, num_buckets) && bucket_starts[num_buckets] == end - begin;
      @ requires_free Sorter.allBucketsInRangeSorted(values, begin, end, bucket_starts, num_buckets, 0, bucket);
      @
      @ // Stays partitioned
      @ requires_free Sorter.allBucketsPartitioned(values, begin, end, bucket_starts, num_buckets);
      @ // All subsequent buckets keep the sorting property
      @ requires_free Sorter.smallBucketsInRangeSorted(values, begin, end, bucket_starts, num_buckets, bucket, num_buckets);
      @ // Equality buckets
      @ requires_free equal_buckets ==>
      ...
      @ requires_free Sorter.notAllValuesInOneBucket(bucket_starts, num_buckets, end - begin);
      @ requires_free \disjoint(storage.allArrays, values[*], bucket_starts[*]);
      @ requires_free \invariant_free_for(storage);
      @ requires \invariant_for(storage);
      @
      @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), \old(\dl_seq_def_workaround(begin, end, values)));
      ...
      @ ensures_free Sorter.allBucketsInRangeSorted(values, begin, end, bucket_starts, num_buckets, 0, bucket + 1);
      @ // Stays partitioned
      @ ensures_free Sorter.allBucketsPartitioned(values, begin, end, bucket_starts, num_buckets);
      @ // All subsequent buckets keep the sorting property
      @ ensures_free Sorter.smallBucketsInRangeSorted(values, begin, end, bucket_starts, num_buckets, bucket + 1, num_buckets);
      @ // Equality buckets at odd indices except the last bucket
      @ ensures_free equal_buckets ==>
      @     Sorter.equalityBucketsInRange(values, begin, end, bucket_starts, num_buckets, bucket + 1, num_buckets - 1);
      @ ensures_free \invariant_free_for(storage);
      @ ensures \invariant_for(storage);
      @
    ...
    /*@ public normal_behaviour
      ...
      @
      @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), \old(\dl_seq_def_workaround(begin, end, values)));
      @ ensures_free Functions.isSortedSlice(values, begin, end);
      @ ensures_free \invariant_free_for(storage);
      @ ensures \invariant_for(storage);
      @
    ...
    private static void sample_sort(int[] values, int begin, int end, Storage storage) {
        ...
        /*@ normal_behaviour
          @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), \old(\dl_seq_def_workaround(begin, end, values)));
          @ ensures_free \disjoint(\all_fields(values), \all_fields(bucket_starts), storage.allArrays);
          @ ensures_free \disjoint(storage.*, storage.allArrays);
          @
          @ assignable_free \strictly_nothing;
          @
          @ measured_by end - begin, 2;
        ...
        /*@ normal_behaviour
          ...
          @ requires_free \disjoint(\all_fields(values), \all_fields(bucket_starts), storage.allArrays, storage.*);
          @
          @ ensures_free \dl_seqPerm(\dl_seq_def_workaround(begin, end, values), \old(\dl_seq_def_workaround(begin, end, values)));
          @ ensures_free Sorter.allBucketsInRangeSorted(values, begin, end, bucket_starts, num_buckets, 0, num_buckets);
          @ ensures_free Sorter.allBucketsPartitioned(values, begin, end, bucket_starts, num_buckets);
          @
          @ assignable_free values[begin..end - 1];
          @ assignable_free storage.allArrays;
        ...
        /*@ normal_behaviour
          @ ensures_free sortednessFromPartitionSorted(values, begin, end, bucket_starts, num_buckets);
          @
          @ assignable_free \strictly_nothing;
          @
          @ measured_by end - begin, 2;
    ...
    /*@ public normal_behaviour
      @ requires_free values.length <= Buffers.MAX_LEN;
      @
      @ ensures_free \dl_seqPerm(\dl_array2seq(values), \old(\dl_array2seq(values)));
      @ ensures_free Functions.isSortedSlice(values, 0, values.length);
      @
      @ assignable_free values[*];
      @*/
    public static void sort(int[] values) {
        Storage storage = new Storage();
        //@ assume \disjoint(storage.allArrays, values);
